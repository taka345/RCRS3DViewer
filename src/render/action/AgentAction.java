...
public class AgentAction {
	public static void drawAmbulanceTeamAction(PApplet applet, int moveX,
			int moveY, float markHeight, PImage actionImage, ViewerConfig config) {
	...
	public static void drawCivilianAction(PApplet applet, int moveX, int moveY,
			float messageHeight, String sayMessage, ViewerConfig config) {
		applet.pushStyle();
	...
	public static void drawFireBrigadeAction(PApplet applet, int posX,
			int posY, int targetX, int targetY, float targetZ,
			ViewerConfig config) {
		...
		case ViewerConfig.LOW:
			...
			applet.bezier(posX, posY, 0, targetX - ((targetX - posX) / 4),
					targetY - ((targetY - posY) / 4), targetZ + (targetZ / 5),
					targetX - ((targetX - posX) / 2), targetY
							- ((targetY - posY) / 2), targetZ + (targetZ / 10),
	...
	public static void drawPoliceForceAction(PApplet applet, int moveX,
			int moveY, float markHeight, int posX, int posY, int targetX,
			int targetY, PImage actionImage, ViewerConfig config) {
	...
	public static void RectPoint(int posX, int posY, int targetX, int targetY,
			PApplet applet) {
		int y0 = 0;
		int x0 = 0;
		...
		if ((targetY - posY) == 0) {
			...
			float hypotenuse = applet.sqrt((targetY - posY) * (targetY - posY)
		...
		applet.quad(pointX1, pointY1, pointX2, pointY2, pointX3, pointY3,
				pointX4, pointY4);
