...
public class BuildingShape extends AreaShape {
	...
	public BuildingShape(Entity entity, ScreenTransform transform,
			...
			PImage[] smokeImage, PImage[] icons) {
	...
	public void drawShape(PApplet applet, ViewerConfig config) {
		drawEffect(applet, x, y, bHeight, super.nords, fireCount, fireImage,
				smokeImage, smoking, burning, smokeCount, config);
	}
	...
	public void drawEffect(PApplet applet, float x, float y, float bHeight,
			...
			PImage[] smokeImage, int smoking, int burning, int smokeCount,
			ViewerConfig config) {
		switch (config.getDetail()) {
		case ViewerConfig.HIGH:
			if (smoking > 0) {
				AreaEffect.drawSmoke(x, y, bHeight, 0, 4, nords, smokeCount, 2,
						smokeImage, applet);
			}
			if (burning > 0) {
				if (smoking > 0) {
					AreaEffect.drawSmoke(x, y, bHeight, smoking, 3, nords,
							smokeCount, -100, smokeImage, applet);
				}
				AreaEffect.drawFire(x, y, bHeight, burning, nords, fireCount,
						-50, fireImage, applet);
			}
			break;
		default:
