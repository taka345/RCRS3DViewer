...
public class BlockadeShape implements EntityShape
{
	...
	private void minmaxXY()//座標�?��?大値と�?小�?�のx,yを設�?
	{
	    for(int i = 0; i < apexes.length; i+=2){
	    	if(i != 0){
	...
	private int setBlockadeScale()//瓦礫の大きさを設�?
	{
	...
	private int setBlockadeNum()//瓦礫の個数を設�?
	{
		if(repairCost <= 35) return 1;
		else return 2;
	}
	private float setRotateY()//瓦礫の回転角を設�?
	{
	    float numX,numY;
	...
	private void blockadeDrawSelect(PApplet applet)//瓦礫の描画の選�?
	{
		switch(blockadeNum){
			case 1 :
