...
public class AreaEffect {
	...
	public static void drawFire(float x, float y, float bHeight, int size,
			float[] nords, int fireCount, int fireInterval, PImage[] fireImage,
			PApplet applet) {
		...
		for (int i = 0; i < nords.length - 2; i += 2) {
			...
			applet.vertex(nords[i], nords[i + 1], bHeight / (size + 1), 0.3f,
					0.7f);
			applet.vertex(nords[i + 2], nords[i + 3], bHeight / (size + 1),
					0.7f, 0.7f);
			applet.vertex(nords[i + 2] + valueNX2, nords[i + 3] + valueNY2,
					bHeight + 60 * ((size + 2) / 2), 0.7f, 0.2f);
			applet.vertex(nords[i] + valueNX, nords[i + 1] + valueNY, bHeight
					+ 60 * ((size + 2) / 2), 0.3f, 0.2f);
			applet.endShape();
		}
		...
		applet.vertex(nords[nords.length - 2], nords[nords.length - 1], bHeight
				/ (size + 1), 0.7f, 0.7f);
		applet.vertex(nords[nords.length - 2] + valueNX2,
				nords[nords.length - 1] + valueNY2, bHeight + 60
						* ((size + 2) / 2), 0.7f, 0.2f);
		applet.vertex(nords[0] + valueNX, nords[1] + valueNY, bHeight + 60
				* ((size + 2) / 2), 0.3f, 0.2f);
		applet.endShape();
		applet.popMatrix();
	}

	public static void drawSmoke(float x, float y, float bHeight,
			int start_point, int size, float[] nords, int smokeCount,
			int smokeInterval, PImage[] smokeImage, PApplet applet) {
		...
		for (int i = 0; i < nords.length - 2; i += 2) {
			...
			applet.vertex(nords[i], nords[i + 1], bHeight / (start_point + 1),
					0.2f, 0.8f);
			applet.vertex(nords[i + 2], nords[i + 3], bHeight
					/ (start_point + 1), 0.8f, 0.8f);
			applet.vertex(nords[i + 2] + valueNX2, nords[i + 3] + valueNY2,
		...
		applet.beginShape(applet.QUAD);
		applet.texture(smokeImage[picNum]);
		applet.vertex(nords[0], nords[1], bHeight / (start_point + 1), 0.2f,
				0.8f);
		applet.vertex(nords[nords.length - 2], nords[nords.length - 1], bHeight
				/ (start_point + 1), 0.8f, 0.8f);
		applet.vertex(nords[nords.length - 2] + valueNX2,
	...
	private static float onNords(float buildingPoint, float nordsPoint,
			int distance) {
		float dis;
	...
	public static void drawText(float x, float y, float z, String str,
			ViewerConfig config, PApplet applet) {
		applet.pushStyle();
	...
	public static void drawAreaIcon(PApplet applet, float centerX,
			float centerY, float areaHeight, int scale, PImage icon,
			ViewerConfig config) {
		applet.pushStyle();
		applet.ambient(255, 255, 255);
		applet.noStroke();
