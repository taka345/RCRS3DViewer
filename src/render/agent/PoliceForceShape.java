...
public class PoliceForceShape extends HumanShape {
	...
	public PoliceForceShape(Entity entity, ScreenTransform transform,
			int scale, PImage actionImage, PImage[] images) {
	...
	public void drawShape(int count, int animationRate, PApplet applet,
		...
		if (!config.getFlag("PoliceForce"))
			return;
		if (super.action) {
			AgentAction.drawPoliceForceAction(applet, super.moveX, super.moveY,
					super.markHeight, posX, posY, targetX, targetY,
					this.policeAction, config);
		}
