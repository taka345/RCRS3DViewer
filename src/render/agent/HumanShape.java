...
public abstract class HumanShape implements EntityShape {
	...
	public void drawShape(int count, int animationRate, PApplet applet,
		...
		if (animationRate <= 0 || history == null || count >= animationRate
				|| history.length <= 3) {
			moveX = this.posX;
			moveY = this.posY;
		} else {
			float rate = (float) animationRate
					/ ((this.history.length / 2) - 1);
			...
			if (this.history.length <= index + 2) {
				...
				float rateX = (this.history[index] - this.history[index + 2])
						/ rate;
				float rateY = (this.history[index + 1] - this.history[index + 3])
						/ rate;
	...
	public void drawAgent(PApplet applet, int moveX, int moveY,
			float direction, int scale, PImage[] images)// High
	{
		int min = scale / 2 * -1;
		int max = scale / 2;
		applet.pushMatrix();
	...
	public void drawHPbar(PApplet applet, int HP, int moveX, int moveY,
			int scale, ViewerConfig config) {
		applet.pushStyle();
	...
	public void drawMarker(PApplet applet, int moveX, int moveY,
			float markHeight, int scale) {
		applet.pushStyle();
		applet.pushMatrix();
		applet.noStroke();
