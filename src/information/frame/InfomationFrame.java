...
public class InfomationFrame {
	...
	private void setPApplet(LocalApplet applet) {
		if(applet != null) {
			applet.setBounds(applet.x + this.x, applet.y +this. height, applet.width, applet.height);
			applet.init();
		}
	...
	public void draw(PApplet p) {
	   	...
	   	try {
	    	for(int i = 0; i < this.label.size(); i++) {
	    		if(this.on.get(i))p.fill(180);
	    		else p.fill(20);
	    		p.rect(x + tempH * i, y, tempH, this.height);

	    		if(this.on.get(i)) p.fill(50);
	    		else p.fill(200);
	    		p.textAlign(PConstants.CENTER, PConstants.CENTER);
	    		p.text(this.label.get(i), x + tempH/2 + tempH * i, this.height / 2);
	...
	public void push(int mouseX, int mouseY)
	{
	    int tempH = this.width/3;
	   	...
	   	for(int i = 0; i < this.on.size(); i++) {
	   		if (checkMousePos(mouseX, mouseY, this.x + tempH * i, this.y, tempH, this.height) && !this.on.get(i)) {
		        index = i;
	    	} else {
	...
	public boolean checkMousePos(int mouseX, int mouseY, int x, int y, int width, int height)
	{
	   	if ((mouseX >= x && x+width >= mouseX) && (mouseY >= y && y+height >= mouseY)) return true;
	    return false;
	}
}
