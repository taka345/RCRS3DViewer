...
{
	protected EntityShape[] shapes; //ビル�?エージェントなどの描画用クラスがここに格納されて�?�?
  ...
  protected StandardWorldModel world; //ワールドモ�?ルビル�?道などの�?報が管�?されて�?�?
  protected StandardWorldModel startWorld;
  protected ChangeSet[] changes; //ワールドモ�?ルの更新�?報が管�?されて�?�?
  protected ScreenTransform transform; //シミュレーション座標を実座標に変換するクラス
  protected Command[][] commands; //[time][id]

  protected int scale; // オブジェクト�?�スケール�?値が大きいと描画が細かくな�?
  protected int startTime; //シミュレーション開始時�?
  protected int endTime; //シミュレーション終�?時間 開始時間と終�?時間は実行中のシミュレーションに接続した�?�合を想定して�?�?
  protected int time; //現在の時刻
  protected double score;
  protected double maxScore;

  protected boolean initialized; //初期化が完�?した�?
  protected boolean updated; //次のス�?�?プへ進んだ�?
  ...
  public void run() //シミュレーション�?報の更新
  {
    
    try {
      while (world == null || transform == null) Thread.sleep(1); //�?報読み込み�?ち

      this.initialize(); //初期�?
      while (!initialized) Thread.sleep(1); //初期化�?ち
      ...
      while (true) {
        ...
        if (this.updated) {
          if (this.changes[time] != null) {
            ...
            time--; //サーバからま�?�?報が�?�信されて�?な�?場合ス�?�?プを進めな�?
          }
          this.updated = false;
          this.onMarkSet = true;
        }
  ...
  protected void createScreenTransform(StandardWorldModel world) //シミュレーション座標を描画用座標に変換するクラスの作�??
  {
