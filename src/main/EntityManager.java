...
public abstract class EntityManager extends Thread {
	...
	private int heatingBuilding = 0, burntoutBuilding = 0,
	...
	public EntityManager(int scale, Config config, PApplet applet,
	...
	protected void createScreenTransform(StandardWorldModel world) // シミュレーション座標を描画用座標に変換するクラスの作成
	{
		...
		ScreenTransform t = new ScreenTransform(b.getMinX(), b.getMinY(),
				b.getMaxX(), b.getMaxY());
		t.rescale(scale, scale);
		this.transform = t;
	}

	public void drawShapes(int count, int animationRate,
			ViewerConfig viewerConfig, CameraParameter camera) {
		if (shapes == null)
			return;
						...
						if (shapes[index] != null) {
							if (shapes[index] instanceof BuildingShape) {
								...
								if (this.onMarkSet) {
									...
									((HumanShape) shapes[index])
											.setMarkHeight(h);
								}
							}
							// 3DViewer Draw Shape!!
							shapes[index].drawShape(count, animationRate,
									applet, viewerConfig);
							// ---------------------------------------------------------
							if (shapes[index] instanceof AreaShape) {
								List<EntityID> bs = ((AreaShape) shapes[index])
										.getBlockades();
								if (bs != null) {
									for (EntityID temp : bs) {
										if (temp == null)
											break;
										Integer bi = indexIDMap.get(temp
												.getValue());
	...
	protected void initialize() // 表示用クラスの準備
	{
		...
		if (blockades == null) {
			blockades = new OBJLoader(this.applet, "data/3dObject/gareki2.obj");
		}

		for (Entity entity : world.getAllEntities()) {
			indexIDMap.put(entity.getID().getValue(), count);
			if (entity instanceof Road) {
				shapes[count] = new RoadShape(entity, transform, this.scale,
						image.getRoadImage(), image.getIcons());
			} else if (entity instanceof Building) {
				// Calculation of the size of the building
				float h = scale / 50;
				Rectangle2D bounds = ((Building) entity).getShape()
						.getBounds2D();
				...
				if (((Building) entity).getTotalArea() > 1000) {
					bHeight = h;
				} else if (((Building) entity).getTotalArea() > 400
						&& ((Building) entity).getTotalArea() <= 1000) {
					bHeight = 4 * h;
				} else {
					bHeight = 2 * h;
				}
				...
				shapes[count] = new BuildingShape(entity, transform, bHeight,
						this.scale, image.getBuildingImage(), image.getFire(),
						image.getSmoke(), image.getIcons());
			} else if (entity instanceof Blockade) {
				shapes[count] = new BlockadeShape(entity, transform,
						this.scale, blockades);
			} else if (entity instanceof FireBrigade) {
				shapes[count] = new FireBrigadeShape(entity, transform, scale,
						image.getFirebrigadeImage());
			} else if (entity instanceof PoliceForce) {
				shapes[count] = new PoliceForceShape(entity, transform, scale,
						image.getPoliceActionImage(), image.getPoliceImage());
			} else if (entity instanceof AmbulanceTeam) {
				shapes[count] = new AmbulanceTeamShape(entity, transform,
						scale, image.getActionAmbulanceImage(),
						image.getAmbulanceImage());
						...
						try {
							...
							if (commands[time] != null) {
								for (int i = 0; i < commands[time].length; ++i) {
									Integer index = indexIDMap
											.get(commands[time][i].getAgentID()
													.getValue());
									if (commands[time][i] instanceof AKClear) {
										AKClear message = (AKClear) commands[time][i];
										Blockade b = (Blockade) world
												.getEntity(message.getTarget());
							...
							this.score = this.scoreFunction.score(world,
									new Timestep(time));
							...
							for (Entity entity : world.getAllEntities()) {
								...
								if (index != null) {
									if (index < shapes.length) {
										shapes[index.intValue()].update(
												world.getEntity(id), transform);
									} else {
										shapeList.get(index - shapes.length)
												.update(world.getEntity(id),
														transform);
										EntityShape eShape = shapeList
												.get(index - shapes.length);
										if (eShape instanceof BlockadeShape) {
											blockadeCount += ((BlockadeShape) eShape)
													.getRepairCost();
										}
									}
								} else {
									// ---------blockade-----------//
									BlockadeShape bs = new BlockadeShape(
											world.getEntity(id), transform,
											this.scale, this.blockades);
									indexIDMap.put(id.getValue(), shapes.length
											+ shapeList.size());
								...
								if (managerlog == null && loop == false) {
									if ((cBlockadeList.get(id)) != null) {
										if (shapes[index.intValue()] instanceof AreaShape) {
											((AreaShape) shapes[index
													.intValue()])
													.setBlockade(cBlockadeList
															.get(id));
										}
									}
								}
								if (entity instanceof Building) {
									...
									if (isRefuge(civ) == true) {
										refugepopulation++;
										((HumanShape) shapes[index.intValue()])
												.setRefuge();
									}
								}
							}
							this.information.setBurnedBuilding(
									time - startTime, fire_count);
							this.information.setPopulation(time - startTime,
									livePopulation);
							this.information.setPopulationData(livePopulation,
									deadPopulation);
							this.information.setBuildingData(heatingBuilding,
									burntoutBuilding, extinguishBuilding);
							this.information.setRefugeData(refugepopulation);
							this.information.setBlockadeCount(time - startTime,
									blockadeCount);
							if (commands[time] != null) {
								for (int i = 0; i < commands[time].length; ++i) {
									Integer index = indexIDMap
									...
									if (index != null) {
										if (commands[time][i] instanceof AKExtinguish) {
											...
											if (shapes[index] instanceof HumanShape) {
												((AmbulanceTeamShape) shapes[index])
														.setCarry();
											}
			...
			if (time >= endTime) {
				if (managerlog != null) {
					try {
						...
						for (Entity e : StandardWorldModel
								.createStandardWorldModel(
										managerlog.getWorldModel(0))
								.getAllEntities()) {
