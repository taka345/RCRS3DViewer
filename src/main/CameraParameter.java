...
class CameraParameter {
	...
	public void camera(PApplet applet) {
		...
		// 0, -1);
		if (viewpoint) {
			applet.camera((float) vp[0] + centerX, (float) ((float) vp[1]
					+ centerY + 0.00043), (float) vp[2], centerX, centerY, 0,
					0, 0, -1);
		} else {
			applet.camera(centerX, (float) (centerY + 0.00043), 20,
					(float) vp[0] + centerX, (float) vp[1] + centerY,
					(float) vp[2], 0, 0, -1);
		}
	}
