...
public class InformationManager extends PApplet {
	...
	public void draw() {
		...
		/*
		 * switch(button.getFlag()) { case 0 : translate(0,40);
		 * gc.setLabelName("Time", "Score"); gc.setTittle("Score");
		 * gc.setVMax(score); gc.setData(score, currentTime); gc.draw(score,
		 * currentTime, 400, 200);
		 * 
		 * translate(0,230); gc.setLabelName("Time", "population");
		 * gc.setTittle("Population"); gc.setVMax(population);
		 * gc.setData(population, currentTime); gc.draw(population, currentTime,
		 * 400, 200);
		 * 
		 * translate(0,230); gc.setLabelName("Time", "Burned Building");
		 * gc.setTittle("Burned Building"); gc.setVMax(numBurnedBuilding);
		 * gc.setData(numBurnedBuilding, currentTime);
		 * gc.draw(numBurnedBuilding, currentTime, 400, 200); break; case 1 :
		 * translate(0,40); refugeData.clear(); refugeData.setValue("Refuge\n"+
		 * refugePopulation, refugePopulation); evacuationPopulation =
		 * allPopulation - refugePopulation - deadPopulation;
		 * refugeData.setValue("Evacuation\n"+ evacuationPopulation,
		 * evacuationPopulation); refugeData.setValue("Dead\n"+ deadPopulation,
		 * deadPopulation); image(new
		 * createChartImage(refugeData).getPieChartPImage(400, 200,
		 * "Refuge"),0,30);
		 * 
		 * translate(0,230); buildingData.clear();
		 * buildingData.setValue("HeatingBuilding\n"+ heatingBuilding,
		 * heatingBuilding); buildingData.setValue("BurntOutBuilding\n"+
		 * burntoutBuilding, burntoutBuilding);
		 * buildingData.setValue("unBurntBuilding\n"+ unBurntBuilding,
		 * unBurntBuilding); image(new
		 * createChartImage(buildingData).getPieChartPImage(400, 200,
		 * "BurnedBuilding"),0,30); break; } popStyle();
		 */
	}
